 ...
 * @generated
 */
public class SupportedByLabelParser implements IParser {
	/**
	* @generated
	*/
	public SupportedByLabelParser() {
	}
	...
	*/
	private IStatus updateValues(EObject target, String newString) throws ExecutionException {
		SupportedBy supportedBy = (SupportedBy) target;
		supportedBy.getImplementationConstraint().get(0).getContent().getValue().get(0).setContent(newString);
		return new Status(IStatus.OK, "acme.argumentation.diagram", "OK");
	}
	...
	*/
	private String evaluatePrintExpression(EObject self) {
		SupportedBy supportedBy = (SupportedBy) self;
		return supportedBy.getImplementationConstraint().get(0).getContent().getValue().get(0).getContent();
	}
