 ...
 * @generated
 */
public class AssuranceCasePackageCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected Set getFeaturesToSynchronize() {
		if (myFeaturesToSynchronize == null) {
			...
			myFeaturesToSynchronize
					.add(assuranceCase.AssuranceCase_Package.eINSTANCE.getAssuranceCasePackage_AssuranceCasePackage());
			myFeaturesToSynchronize
					.add(assuranceCase.AssuranceCase_Package.eINSTANCE.getAssuranceCasePackage_ArgumentPackage());
			myFeaturesToSynchronize
					.add(assuranceCase.AssuranceCase_Package.eINSTANCE.getAssuranceCasePackage_ArtifactPackage());
			myFeaturesToSynchronize
					.add(assuranceCase.AssuranceCase_Package.eINSTANCE.getAssuranceCasePackage_TerminologyPackage());
		}
	...
	*/
	...
	protected List getSemanticChildrenList() {
		...
		List<assuranceCase.diagram.part.AssurancecaseNodeDescriptor> childDescriptors = assuranceCase.diagram.part.AssurancecaseDiagramUpdater
				.getAssuranceCasePackage_1000SemanticChildren(viewObject);
		for (assuranceCase.diagram.part.AssurancecaseNodeDescriptor d : childDescriptors) {
	...
	*/
	private boolean isMyDiagramElement(View view) {
		int visualID = assuranceCase.diagram.part.AssurancecaseVisualIDRegistry.getVisualID(view);
		switch (visualID) {
	...
	*/
	protected void refreshSemantic() {
		...
		List<assuranceCase.diagram.part.AssurancecaseNodeDescriptor> childDescriptors = assuranceCase.diagram.part.AssurancecaseDiagramUpdater
				.getAssuranceCasePackage_1000SemanticChildren((View) getHost().getModel());
		...
		for (Iterator<assuranceCase.diagram.part.AssurancecaseNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			assuranceCase.diagram.part.AssurancecaseNodeDescriptor next = descriptorsIterator.next();
			String hint = assuranceCase.diagram.part.AssurancecaseVisualIDRegistry.getType(next.getVisualID());
		...
		for (assuranceCase.diagram.part.AssurancecaseNodeDescriptor next : childDescriptors) {
			String hint = assuranceCase.diagram.part.AssurancecaseVisualIDRegistry.getType(next.getVisualID());
